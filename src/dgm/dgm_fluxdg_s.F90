...
!> $URL: https://min3psvn.ubc.ca/svn/min3p_thcm/branches/dsu_new_add_2024Jan/src/dgm/dgm_fluxdg_s.F90 $
!---------------------------------------------------------------------
!********************************************************************!
    
!c ----------------------------------------------------------------------
!c subroutine dgm_fluxdg_s
!c --------------------------
!c
!c compute diffusive flux of gas species using the dusty gas model for rhs vector
...
!c           dgm_gflux_s(ng)    = total molar diffusive fluxes for    * +
!c                                gas components [moles m^-2 d^-1]
!c           dgm_neflux_s(ng)   = non-equimolar diffusive fluxes for  * +
!c                                gas components [moles m^-2 d^-1]
!c
!c           integer*4:
!c           ----------
...
!c ----------------------------------------------------------------------
      ...
      subroutine  dgm_fluxdg_s(c_i         ,c_j          ,    &
                               c_ij        ,molfrac      ,    &
                               zgi         ,zgj          ,    &
                               dens        ,gpij         ,    &
                               tempkel     ,permijy      ,    &
...
!c     passed
      real*8       c_i(ng)         ,c_j(ng)        ,    &
                   ...
                   small           ,                    &
                   amat(ng,ng)     ,bmat(ng)       ,    &
                   dgm_gflux_s(ng) ,dgm_neflux_s(ng)    

      integer*4    ipvt(ng)

!c     internal
      integer*4    i, ic, jc, ig, info
    
      real*8       diff_knud, neflux, alpha_diff, permijx
      
      real*8       expt, beta, vis2, gfm2 
...
!c         knudsen term
          ...
          diff_knud   = permijx / visc_comp_g(ic)                      & 
                      * beta * (permijx**expt)                         &
                      * visc_comp_g(ic) / vis2                         &
                      * dsqrt ( gfm2 / gfwg(ic) )                      &
                      * sec_per_days 

          amat(ic,ic) = amat(ic,ic) - 1.d0 / diff_knud !(ic) !/ gporx
...
!c     initialize dgm_gflux_s
      do ig=1,ng
        dgm_gflux_s(ig) = bmat(ig)
        dgm_neflux_s(ig) = neflux * molfrac(ig)
       enddo
