...
!> $URL: https://min3psvn.ubc.ca/svn/min3p_thcm/branches/dsu_new_add_2024Jan/src/min3p/readminx.F90 $
!---------------------------------------------------------------------
!********************************************************************!

!c ----------------------------------------------------------------------
!c subroutine readminx
!c -------------------
!c
...
!c ----------------------------------------------------------------------
      ...
      subroutine readminx(imdbs,ipsp,ilog,idbg)
      ...
      integer :: imdbs,ipsp,ilog,idbg
      
      integer :: i, ic, icount, icur, imx, istart, iend, iv,           &
...
!c  constituting mineral
          ...
          read(imdbs,*,err=997) string1, string2
          read(imdbs,*,err=997) junk
          read(imdbs,*,err=997) junk

          string1 = string1

          if (string2.eq.'reversible' .or.                            &
     &        string2.eq.'raoult' .or.                                &
     &        string2.eq.'dissolution_to_equilibrium' .or.            &
     &        string2.eq.'precipitation_to_equilibrium') then
                                                                       
            read(imdbs,*,err=997) eqt,nv,                             &
...
!c  read equilibrium constant and reaction stoichiometry
     ...
     &                       '"far from equilibrium"' 
              ...
              if (temp_corr) then                                        
                read(imdbs,*,err=997) junk,junk,dhcmx(imx)
              else                                                       
                read(imdbs,*,err=997) junk
              end if                                                     
              read(imdbs,*,err=997) junk
              read(imdbs,*,err=997) eqmxs(imx),nv,                      &
     &                             (namet(iv),xnumt(iv),iv=1,nv)
...
!c  exit if end of file is reached and mineral specified was not found
            ...
            elseif (name.eq.'end') then
              if (rank == 0) then
                write(ilog,'(72a)') ('-',i=1,72)
                write(ilog,'(3a)') 'mineral ',namemx(imx),      &
...
997   continue
      ...
      if (rank == 0) then
        write(ilog,*) 'SIMULATION TERMINATED'
        write(ilog,'(2a)') 'error reading in mineral database (readminx.F90): ',trim(strbuffer)
        write(*,*) 'SIMULATION TERMINATED'
        write(*,'(2a)') 'error reading in mineral database (readminx.F90): ',trim(strbuffer)
        close(ilog)
      end if
#ifdef PETSC
      call petsc_mpi_finalize
