...
!> $URL: https://min3psvn.ubc.ca/svn/min3p_thcm/branches/dsu_new_add_2024Jan/src/min3p/ddbdflux.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c ----------------------------------------------------------------------
      ...
      integer :: i1, i2, ibvs, icon, istart, iend, jvol
      real*8 :: delp_dd, delz_dd
      real*8, external :: fluxdd
      
      real(type_r8) :: rho_av_loc
      real(type_r8) :: vsflux_icon_loc
      real(type_r8) :: relperm_av, visco_av
      ...
      if ((btypevs(ibvs).eq.'first').or.             &  !first type (Dirichlet)
        ...
        do i1=istart,iend         !loop over connections
          ...
          if (delz_dd .ne. r0) then
            ...
            if (av_dens_z) then
              delp_dd = rho_av_loc*(uvsnew(jvol)/density(jvol)-        &
...
!c  assign coefficients for upstream weighting and central weighting
            ...
            if (.not. is_cell_based_relp) then
              if (upstream) then
                if (delp_dd .gt. r0) then
                  relperm_av = relperm(jvol)
                  visco_av = viscosity(jvol)
                else          
                  relperm_av = relperm(ivol)
                  visco_av = viscosity(ivol)
                end if
              else
                relperm_av = rhalf*(relperm(ivol)+relperm(jvol))
                visco_av = rhalf*(viscosity(ivol)+viscosity(jvol))
              end if
            end if
!cprovi-----------------------------------------------------------                   
!cprovi-----------------------------------------------------------                   
...
!cdsu calculate influence coefficient for variable saturated flow
              ...
              vsflux_icon_loc = -fluxdd_usg(delp_dd,num_edge_dvols,ncell,           &
              ...
              vsflux_icon_loc = - fluxdd(delp_dd,cinfvs_a(i1),dcoef_loc)
