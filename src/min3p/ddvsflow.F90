...
!> $URL: https://min3psvn.ubc.ca/svn/min3p_thcm/branches/dsu_new_add_2024Jan/src/min3p/ddvsflow.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
#include <finclude/petscsys.h>
      ...
      integer :: i, i2, ierr, ilist, iter, ivol, izn, ipest,           &
                 istatus, maxiter, n_unknown_vs, n_unknown_glob,       &
                 ilun_pest, iter_div, ibvs
      real*8 :: cputime, errormax, uvsmax
...
#ifdef USG
      integer :: icell
      real*8 :: rdummy, uvsnew_cell, sanew_cell, sgnew_cell, snnew_cell
#endif
...
!c  compute soil hydraulic parameters
    ...
    !$omp num_threads(numofthreads_global)                            &
    !$omp default(shared)                                             &
    !$omp private (icell,ivol,izn,i2,                                 &
    !$omp uvsnew_cell,sanew_cell,sgnew_cell,snnew_cell)
    !$omp do schedule(static)
#endif
...
#endif
      end if                                       !disable saturation update
...
!cprovi--------------------------------------------        
          if (heat_transport .and. decoupled_type_vs_heat <= 1) then
            ...
            ittot_vs = ittot_glob            !set ittot_vs to ittot_glob as ittot_vs is the variable 
...
!cdsu--------------------------------------------------------------------
!cdsu assemble matrix and rhs-vector for flow and heat equation  
!cdsu   modified jacdd_energybal to account for variably saturated conditions
...
100             prt_flow_symbfac_comp = cputime()
                ...
                call pardiso_substitution(ilog, msolvit_glob,          &
                     itsolv, idetail_glob, res_glob, restol_glob,      &
                     deltol_glob, over_flow, rnorm, rmupdate,          &
              ...
              if(b_solver_test_petsc) then
                ...
                call solver_dd_snes_solve_flow_heat(ilog,.true.,       &
                          ...
                          iaglob,jaglob,2*nngl,itsolv,                 &
                          over_flow,rnorm,row_idx_l2pg_glob,           &
              ...
              if(b_solver_test_lis) then
                !only solver the local part, update the ghost value
                call solver_lis_solve_flow(ilog,idetail_glob,          &
                            aglob,bglob,uglob_std,                     &
                            iaglob,jaglob,2*nngl,2*nn,2,               &
                ...
                if(rank == 0 .and. b_enable_output) then
                  write(*,'(a, 2(1x, i5), 1x, l1, 1x, 1pe15.6e3)')       &
                       "ddvsflow-A2: rank, iteration, over_flow, rnorm ",&
                       rank, itsolv, over_flow, rnorm  
                end if
#endif
              end if
...
!c  solve [avs] * {uvs} = {bvs}
              ...
              call ws209(ilog,2*nngl,msolvit_glob,itsolv,                   &
                   idetail_glob,iaglob,jaglob,iafglob,iafdglob,jafglob,     &
                   lorderglob,aglob,afglob,uglob,bglob,res_glob,rwork_max,  &
                   restol_glob,deltol_glob,njaglob,njafglob,over_flow,rnorm,&
...
#ifdef PARDISO
              if (b_solver_test_pardiso) then
                call solver_results_check_output(ittot_glob, 2*nngl,       &
              ...
              if (b_solver_test_lis) then        
                call solver_results_check_output(ittot_glob, 2*nngl,     &
                     uglob, uglob_std, "ddvsflow_glob_lis") 
              end if
#endif
...
200           prt_flow_symbfac = cputime() 
              ...
              call pardiso_substitution(ilog, msolvit_glob,            &
                   itsolv, idetail_glob, res_glob, restol_glob,        &
                   deltol_glob, over_flow, rnorm, rmupdate, iparm_glob,&
                   ptglob, 2*nngl, njaglob, iaglob, jaglob_std,        &
            ...
            !! use LIS solver
              ...
              if(rank == 0 .and. b_enable_output) then
                write(*,'(a, 2(1x, i5), 1x, l1, 1x, 1pe15.6e3)')       &
                    "ddvsflow-B2: rank, iteration, over_flow, rnorm ", &
                    rank, itsolv, over_flow, rnorm  
              end if
#endif
#endif        
...
!              call export_mmformat_gbl(2*nn,2*nngl,njaglob,iaglob,     &
!                   jaglob,aglob,bglob,uglob,.true.,.true.,.true.,      &
!                   "ddvsflow_glob_check",nngl,nngbl,.false.,ittot_glob)
!            end if
!
...
!              end do
!            
!              !c IMPORTNANT NOTE: convert from pressure head to pressure if the results are obtained vsflow
!              !uglob(1:nngl) = uglob(1:nngl)*ref_dens*gacc
!
!              !c read temperature data
!              uglob(nngl+1:2*nngl) = 0.0d0
!              nskip = 0
!              do ivol = 1, nngl
!#ifdef  PETSC
!                do iskip = 1, node_idx_lg2g(ivol) - nskip -1
...
300             prt_flow_symbfac_comp = cputime()
                if(bsymbolicfactor_vs  .or.i_symfactor_type_flow == 1) then
                ...
                if (b_redo_symbfac .and.(itsolv > n_max_iteration_flow .or.&
                  rnorm > r_max_residual_flow .or. over_flow)) then
              ...
              if(b_solver_test_lis) then    
                ...
                if(rank == 0 .and. b_enable_output) then
                  write(*,'(a, 2(1x, i5), 1x, l1, 1x, 1pe15.6e3)')          &
                        "ddvsflow-C2: rank, iteration, over_flow, rnorm ",  &
                        rank, itsolv, over_flow, rnorm
                end if
#endif
              end if
...
400           prt_flow_symbfac = cputime()
            ...
            !! use LIS solver
              ...
              if(rank == 0 .and. b_enable_output) then
                write(*,'(a, 2(1x, i5), 1x, l1, 1x, 1pe15.6e3)')          &
                      "ddvsflow-D2: rank, iteration, over_flow, rnorm ",  &
                      rank, itsolv, over_flow, rnorm
              end if
#endif
#endif        
...
#ifdef PETSC
                call export_mmformat_gbl(nn,nngl,njavs,iavs,javs,avs,  &
                     bvs,uvs,.false.,.false.,.true.,"ddvsflow_vs",nngl,&
...
!cdsu  debug part, use external solution (written in sequential order) to test
!            if (mtime >= 1 .and. mtime <= 3 .and. ittot_vs <= 30) then
!              call export_mmformat_gbl(nn,nngl,njavs,iavs,javs,        &
!                          avs,bvs,uvs,.false.,.false.,.true.,          &
!                          "vsflow_vs_check",nngl,nngbl,.false.,ittot_vs)
!
!              if (rank == 0) then
!                write(*,*) "-->read hydraulic head and temperature change from external"
!              end if
              ...
              if (reduce_timestep) then
                ...
                if (steady_flow) then
                  if (rank == 0) then
                    ...
                    '-------------------------------------------'
                    write(ilog,*)                                      &
                    '   terminated in routine ddvsflow          '
                    write(ilog,*)                                      &
