...
!> $URL: https://min3psvn.ubc.ca/svn/min3p_thcm/branches/dsu_new_add_2024Jan/src/phase_class/tools/xpath/m_path.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
character(len=200)         :: ancestor_path           ! local variable
     ...
     if (status == 0) then
        ...
        if (debug) print *, "Setting ancestor_path to: ", trim(ancestor_path)
     endif
...
if (path_present) then
   if (debug) print *, "SEARCHING for: ", trim(path)
   if (path(1:1) /= "/") then
      !
      ...
      if (ancestor_path == "") then
        if (debug) print *, "Relative search with null ancestor..."
      endif
      relative_path = .true.    
      if (debug) print *, "Relative search. ANCESTOR ELEMENT: ", &
                             trim(ancestor_path)
      !
      ! Convert to absolute path
      local_path = trim(local_path) // "/" // trim(path)
      if (debug) print *, "Converting ", trim(path), &
...
do ! Loop until we satisfy the constraints 
   ...
   if (debug) print *, "--> Calling get_path ..."
   call get_path(fxml,local_path,local_attributes,pcdata,local_status)
   if (debug) print *, "-->Status after get_path: ", local_status
   if (local_status /= 0)  EXIT

   if (debug) print *, "FOUND path matching: ", trim(local_path)

   if (att_name_present) then
      if (debug) print *, "Checking ", trim(att_name), " among ", &
                          number_of_entries(local_attributes), " entries:"
      if (debug) call print_dict(local_attributes)
      ...
      if (has_key(local_attributes,att_name)) then
         ...
         if (att_value_present) then
            ...
            if (local_status /= 0) then
               if (debug) print *, "Failed to get value of att: ", &
                                    trim(att_name)
               EXIT
            endif
            ...
            if (att_value == value)  then
               local_status = 0
               if (debug) print *, "Got correct att name and value "
               EXIT
            else
               if (debug) print *, "att value: ", trim(value), &
                                   " does not match"
               cycle        ! We keep searching
            endif
         else           ! Found att_name, and no value required
            local_status = 0
            if (debug) print *, "Got correct att name"
            EXIT
         endif
      else              ! Did not find that attribute name
         if (debug) print *, "Att name not present"
         cycle          ! keep searching
      endif
   else                 ! Found path, and no att info required
      local_status = 0
      if (debug) print *, "Found correct path. No other reqs."
      EXIT
   endif
...
if (present(status)) then
   status = local_status
   if (debug) print *, "--Returning status: ", status
endif
...
!
   ...
   if (debug) print *, "Max length of pcdata store: ", max_len_pcdata
endif

if (looking_for_current_element) then
   if (debug) print *, "Returning info about current element"
...
if (eof_xmlfile(fxml)) then
   ...
   if (debug) print *, "Found end of file"
   if (pcdata_requested) pcdata = ""
else if (global_status == END_OF_ANCESTOR_ELEMENT) then
   if (debug) print *, "Found end of ancestor element"
   if (pcdata_requested) pcdata = ""
else
   if (debug) print *, "Parser found candidate element"
   if (pcdata_requested) then
      pcdata = pcdata_recovered(1:len_pcdata)
      if (debug) print *, "PCDATA recovered: ", pcdata_recovered(1:len_pcdata)
   endif
endif
if (global_status > 0) then
   if (debug) print *, "Something went slightly wrong. Status > 0"
endif
!
...
character(len=100)   :: path              ! *** Hard limit

call xml_path(xp,path)
if (debug) print *, " begin_element ::: PATH: " ,  trim(path)
if (debug) print *, "path: ", trim(path), " req: ", trim(path_required)
if (match(path,path_required)) then
      if (debug) print *, " Match path: " ,  trim(path)
      in_target_element = .true.
      target_path = path
      in_pcdata_level = .true.
      if (debug) print *, "In element name: " , name
      if (attributes_requested) attributes_recovered = attributes
      ! stop parsing
      if (debug) print *, "full_node: ", full_node
      if (.not. full_node)  then
         if (debug) print *, "Stopping parsing after initial tag"
         stop_parsing = .true.
      endif
else 
   !
...
if (in_target_element) then
   if (path == target_path) then
      ...
      !
      if (debug) print *, "Exiting target element: ", trim(target_path)
      in_target_element = .false.
      in_pcdata_level = .false.
      if (debug) print *, "Stopping parsing after end of target element"
      stop_parsing = .true.
   endif
   ...
   if (match(left_path,ancestor_path)) then
      ...
      !
      if (debug) print *, "Relative search. End of element: ", name
      if (debug) print *, "Leaving Path: ", trim(left_path)
      if (debug) print *, "Ancestor Path: ", trim(ancestor_path)
      if (debug) print *, "Stopping parsing after end of ancestor element"
...
call xml_path(xp,path)
if (debug) print *, " empty_element ::: PATH: " ,  trim(path)
if (debug) print *, "path: ", trim(path), " req: ", trim(path_required)
if (match(path,path_required)) then
      if (debug) print *, " Match path: " ,  trim(path)
      if (debug) print *, "In (empty) element name: " , name
      if (attributes_requested) attributes_recovered = attributes
      ! stop parsing
      stop_parsing = .true.
      if (debug) print *, "Stopping parsing after empty tag"
      if (debug) print *, "full_node: ", full_node
      if (full_node)  then
         if (debug) print *, "*Warning: full_node requested, empty tag found"
      endif
endif
!
...
if (in_pcdata_level) then
   ...
   !
   if (pcdata_requested) then
      if (debug) print *, "Found chunk of pcdata: ", chunk
      len_chunk = len(chunk)
      if ((len_pcdata + len_chunk) > max_len_pcdata) then
         !
         if (debug) print *, "***Pcdata Overflow "
