...
!> $URL: https://min3psvn.ubc.ca/svn/min3p_thcm/branches/dsu_new_add_2024Jan/src/min3p/updtbcvs.F90 $
!---------------------------------------------------------------------
!********************************************************************!

!c ----------------------------------------------------------------------
!c subroutine updtbcvs
!c -------------------
!c
!c update boundary conditions (variably saturated flow) 
!c
...
!c -> define area (only on surface of solution domain)
!c
!c written by:      Uli Mayer - April 17, 01
!c
...
!c ----------------------------------------------------------------------
      ...
      integer :: i, ibz, ibvs, ierr, ivol, l_string,  istart, iend,    &
                 ierrcd, jtemp, iflag, itype_grad
      real*8 :: areaf, dummy, xbmin, xbmax, ybmin, ybmax, zbmin, zbmax,&
                pos_grad, timebcvsloc
      ...
      if ((.not. b_interpolation_bcvs .and.                            &
          time_check >= time_bcvs_prev .and. time_check < time_bcvs) .or.    &
            ...
            do ibvs = 1, nbvs
              ...
              if (btypezn.eq.'first') then
                ...
                if (.not. b_restart_update_bcvs) then
                  hhead(ivol) = bcondvs(ibvs) + zg(ivol)
...
!  gradient type condition - variable pressure head at different locations
              elseif (btypezn.eq.'gradient')then
                ...
                if (b_interpolation_bcvs) then
                  if (.not.b_updt_next_only .or. b_restart_update_bcvs) then
                    bcondvs_prev(nbvs) = rwork(1,ibz) + rwork(2,ibz)*pos_grad
                  end if
                  bcondvs_next(nbvs) = rwork_next(1,ibz) + rwork_next(2,ibz)*pos_grad
                  bcondvs(nbvs) = math_common_linear(                  &
                                       time_bcvs_prev, time_bcvs,      &
                                       bcondvs_prev(nbvs),             &
                                       bcondvs_next(nbvs),time_check)
                else
                  bcondvs(nbvs)=rwork(1,ibz) + rwork(2,ibz)*pos_grad
                end if

                if (hydraulic_head) then
                  bcondvs(nbvs) = bcondvs(nbvs) - zg(ivol)                
                end if

                if (.not. b_restart_update_bcvs) then
                  hhead(ivol) = bcondvs(nbvs) + zg(ivol)
                  if (fully_saturated) then
                    uvsnew(ivol) = hhead(ivol)
                    uvsold(ivol) = hhead(ivol)
                  elseif (variably_saturated) then
...
!c  first type boundary condition - constant pressure head
              ...
              if (btypezn.eq.'first') then
                ...
                if (pressure_head) then
                  bcondvs(nbvs) = rwork(1,ibz)
                elseif (hydraulic_head) then
                  bcondvs(nbvs) = rwork(1,ibz) - zg(ivol)                
                end if
                  ...
                  if (cell_type == cell_type_tri .or.                  &
                        ...
                        do icell = 1, janumcell(jtemp)
                          ...
                          if (icell2 > 0) then
                            do idvol = 1, num_edge_dvols
                              call get_cell_edge_cvol_id(idvol,        &
                                       ivol,jvol,icell2,idvol_r,iedge_r)
                    ...
                    if (rank == 0) then
                      write(ilog,'(2a,i2)') 'boundary surface area calculation ',&
                            'does not support cell_type updtbcvs',cell_type
                    end if
#ifdef PETSC
                    call petsc_mpi_finalize
#endif
                  ...
                  if (tkel(ivol) > pressure_melt_k(ivol,r0)) then
                    if (b_freezing_adjacent_bd) then
                      if (freezing_adjacent_bd(ivol)) then
                        ...
                        bcondvs(nbvs) = areaf*rwork(1,ibz)*sec_per_days
                      end if
                    else
                      bcondvs(nbvs) = areaf*rwork(1,ibz)*sec_per_days
                    end if
                  ...
                  if (tkel(ivol) > pressure_melt_k(ivol,r0)) then
                    if (b_freezing_adjacent_bd) then
                      if (freezing_adjacent_bd(ivol)) then
                        ...
                        bcondvs(nbvs) = rwork(1,ibz)*sec_per_days
                      end if
                    else
                      bcondvs(nbvs) = rwork(1,ibz)*sec_per_days
                    end if
