...
!> $URL: https://github.com/min3p-ubc/min3p/blob/main/src/min3p/opnenergybal_sub.F90 $
!---------------------------------------------------------------------
!********************************************************************!

!c ----------------------------------------------------------------------
!c subroutine opnenergybal_sub
!c -------------------
!c
        ...
        if (rank == 0 .and. b_enable_output) then
          if (b_output_trans_binary) then
            ...
            call binary_file_open(PETSC_COMM_SELF,               &
                         imheat_mpi(imheat(isub)), prefix(:l_prfx)//'_o.ebal', &
                         .true.)
          else
...
!c  version information
        ...
        if (rank == 0 .and. b_enable_output) then     
          ...
          write(ifls,'(a/72a/)')'subdomain system energy',('-',i=1,72)
...
#endif
            call binary_file_open(PETSC_COMM_SELF,               &
                         imheat_mpi(imheat(isub)), prefix(:l_prfx)//'_o.ebalc',&
                         .true.)
          else
            b_rewind_valid = check_rewind_status(prefix(:l_prfx)//'_o.ebalc')
            if (b_rewind_valid .and. i_append_sim > 0) then
              open(imheat(isub),file=prefix(:l_prfx)//'_o.ebalc',status='unknown',&
                   form='formatted',position='rewind')
            else
              open(imheat(isub),file=prefix(:l_prfx)//'_o.ebalc',status='unknown',&
                   form='formatted')
...
!c  version information
        ...
        if (rank == 0 .and. b_enable_output) then
          ...
          write(ifls,'(a/72a/)')'subdomain energy balance contributions',&
                                ('-',i=1,72)
...
#endif
            call binary_file_open(PETSC_COMM_SELF,               &
                         imheat_mpi(imheat(isub)), prefix(:l_prfx)//'_o.ebale',&
                         .true.)
          else
