...
!> $URL: https://min3psvn.ubc.ca/svn/min3p_thcm/branches/dsu_new_add_2024Jan/src/min3p/drategas.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c ----------------------------------------------------------------------
      ...
      subroutine drategas(g,tkel,hhead,zg,drtinc,tid)
 
      use parm
      use chem
      use bbls
      use mip_bubble, only : mip_mt_enable
      use gen, only : gacc, rank, ilog, b_enable_output
...
!c  compute confining pressure
      ...
      pres_conf = pres_atm    &
               + (rho_w * gacc * (hhead - zg))/pa_atm

!c  compute total rate of degassing, if sum of partial pressures 
!c  exceeds confining pressure

!c  the following code was modified for gas bubble in the isotope code
...
!c  escaping gas mixture
          ...
          do ig = 1,ng
            rateg(ig,tid) = (rgasatm*tkel*ginc(ig,tid)/pres_tot_inc *  &
                rate_inc - rgasatm*tkel*g(ig)/pres_tot * rate) / drtinc
          end do
        end if
      end if

!!c  the following code is used in the original ebullition code.
!!c  change back to the original code since isotope code contains some bugs 
!!c  for gas bubble model. DSU, 2020-04-22
!                
!      if (pres_tot.gt.pres_conf) then
!
!        rate = -degas_rate * (r1-pres_tot/pres_conf)
