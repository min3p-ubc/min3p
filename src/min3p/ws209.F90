...
!> $URL: https://min3psvn.ubc.ca/svn/min3p_thcm/branches/dsu_new_add_2024Jan/src/min3p/ws209.F90 $
!---------------------------------------------------------------------
!********************************************************************!

!c! THIS FILE IS DEPRECATED, PLEASE USE SRC/SOLVER/WS209.F90 INSTEAD
...
!cintellinux  *** relapce isnan by huge = 1.0d300 ***
        ...
        if (numit == nitmax .and. .not.converged) then
          over_flow = .true.
          if(idetail.GT.i1) then
            write(lfile,'(1x,a)') "Linear solver failed due to maximum iteration"
          end if
        end if
     
      ELSE IF(rnorm.eq.r0) THEN

        IF (idetail.GT.i1) THEN        !rnorm.le.r0 -> converged
          WRITE (lfile,14)
        END IF
        converged = .TRUE.
      ELSE
        if(b_output .and. idetail.GT.i1) then
          WRITE (lfile,15)
        end if
        over_flow = .true.
      ENDIF
